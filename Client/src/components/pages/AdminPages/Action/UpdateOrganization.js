...
import { getOrganizationBYid, getStudentBYid, getStudentBYids, host1, updateUser } from '../../../../utils/api';
  ...
  const onChange=(e)=>{
    ...
    useEffect(()=>{
        if(!id){
            ...
            axios.get(getOrganizationBYid+id,{
                headers: {
                    'Authorization':`Bearer ${JSON.parse(localStorage.getItem("login")).AccessToken}`
                }}).then(res=>{
  ...
  return (
    <UpdateStud>
      <ToastContainer
        ...
        <input type="hidden" value={'ORGANIZATION'} name="role" />
            <h1 className='text-white pb-5'>UPDATE ORGANIZATION</h1>
            <div className='row'>
              <div className='col-sm-12 pb-4'>
                <div className='form-group'>
                  <label>Name:</label>
                  <Inputs type="text" defaultValue={student.Name} className="form-control" placeholder="Name" name="Name" onChange={onChange} required/>
                </div>
              </div>
