  ...
  (req, res, next) => {
    ...
    if (req.files) {
      for (var i = 0; i < req.files.length; i++) {
        reqFiles.push(
          "https://eboardfrontendapplication.herokuapp.com/uploads/file/" + req.files[i].filename
        );
      }
    }
...
router.get("/findByIdTheme/:id", (req, res) => {
  Courses.find({ idTheme: req.params.id })
  .populate({
    path: "idOwner",
    populate: "User" ,
  })
