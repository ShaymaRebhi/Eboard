var CommentCourse = require("../Model/CommentCourse");
...
module.exports = {
    ...
    const cmntcours = await CommentCourse.find({Course :  mongoose.Types.ObjectId(req.params.id)}).populate({
      path: "Writer",
      populate: "User" ,
    });
...
getCourseBytask : async(req,res,next) =>{
  try {
  const  id  = req.params.id;
  const cmntcours = await CommentCourse.find({Task :  mongoose.Types.ObjectId(req.params.id)}).populate({
    path: "Writer",
    populate: "User" ,
  });;
